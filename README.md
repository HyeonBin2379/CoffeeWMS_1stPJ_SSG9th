...
[상세내용](https://santiago-kwp.github.io/hyeonbin-lee/2025-10-08-post-hyeonbin/)

#### 리팩토링할 내용 및 다음 프로젝트에서 개선할 사항

- 로그인/회원관리 기능 관련 수정할 사항(다음 프로젝트에서 반영할 사항)
  - 로그인/로그아웃 시 세션 적용 -> 회원관리 기능 관련 SQL 쿼리문 변경 필요
  - 일반회원(Members)/관리자(Managers) 테이블을 회원정보(Users) 테이블에 관한 뷰로 대체
  - 회원정보 테이블에 회원탈퇴날짜 속성 추가
- Member 또는 Manager 객체 생성 시 빌더 패턴 적용(완료)
- 아래의 인증기준을 적용한 비밀번호, 핸드폰번호, 이메일 등의 민감정보 마스킹(일부 완료)
